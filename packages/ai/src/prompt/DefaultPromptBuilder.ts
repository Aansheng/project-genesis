...
export class DefaultPromptBuilder implements PromptBuilder {
  ...
  constructor(
    ...
    if (rendererOrOptions !== undefined && !('render' in rendererOrOptions)) {
      ...
      this.promptAssemblyTraceRenderer = opts.promptAssemblyTraceRenderer
      ...
      this.promptAssemblyTimelineExporter = opts.promptAssemblyTimelineExporter
      ...
      this.promptAssemblyTraceRenderer = undefined
      ...
      this.promptAssemblyTimelineExporter = undefined
  ...
  async build(context: PipelineContext): Promise<AIRequest> {
    ...
    // Phase 0.955: PromptAssemblyPlanner — build section priority plan
    ...
    // Phase 0.959: PromptInspectorBuilder — build inspector from snapshot
    let promptInspector: PromptInspector | undefined
    if (promptAssemblySnapshot !== undefined && this.promptInspectorBuilder !== undefined) {
      promptInspector = this.promptInspectorBuilder.build(promptAssemblySnapshot)
    }

    // Phase 0.9595: PromptInspectorRenderer — render inspector to human-readable string
    let inspectorRendered: string | undefined
    if (promptInspector !== undefined && this.promptInspectorRenderer !== undefined) {
    ...
    // Phase 0.9599: PromptAssemblyTraceRenderer — render trace as human-readable string
    let traceRendered: string | undefined
    if (trace !== undefined && this.promptAssemblyTraceRenderer !== undefined) {
      traceRendered = this.promptAssemblyTraceRenderer.render(trace)
    }

    // Phase 0.95995: PromptAssemblyTraceExporter — export trace as serialized JSON
    let traceExported: string | undefined
    if (trace !== undefined && this.promptAssemblyTraceExporter !== undefined) {
      traceExported = this.promptAssemblyTraceExporter.export(trace)
    }

    // Phase 0.95996: PromptAssemblyTimelineBuilder — build timeline from current trace
    let timeline: import('../strategy/PromptAssemblyTimeline').PromptAssemblyTimeline | undefined
    if (trace !== undefined && this.promptAssemblyTimelineBuilder !== undefined) {
      timeline = this.promptAssemblyTimelineBuilder.build([trace])
    }

    // Phase 0.95997: PromptAssemblyTimelineDiffer — diff empty timeline vs current timeline
    let timelineDiff: import('../strategy/PromptAssemblyTimelineDiff').PromptAssemblyTimelineDiff | undefined
    if (timeline !== undefined && this.promptAssemblyTimelineDiffer !== undefined) {
      timelineDiff = this.promptAssemblyTimelineDiffer.diff(
        { entries: [] },
        timeline,
      )
    }
    ...
    // Phase 0.959976: PromptAssemblyTimelineExporter — export timeline as JSON string
    let timelineExported: string | undefined
    if (timeline !== undefined && this.promptAssemblyTimelineExporter !== undefined) {
      timelineExported = this.promptAssemblyTimelineExporter.export(timeline)
    }

    // Phase 0.9599765: PromptAssemblyTimelineSnapshotBuilder — build timeline snapshot
    let timelineSnapshot: import('../strategy/PromptAssemblyTimelineSnapshot').PromptAssemblyTimelineSnapshot | undefined
    if (timeline !== undefined && this.promptAssemblyTimelineSnapshotBuilder !== undefined) {
    ...
    // Phase 0.9599767: PromptAssemblyHistoryBuilder — build history from current trace
    let history: PromptAssemblyHistory | undefined
    if (trace !== undefined && this.promptAssemblyHistoryBuilder !== undefined) {
    ...
    // Phase 0.9599775: PromptAssemblyObservatoryDiffer — diff empty baseline vs current observatory
    ...
    const metadata: Record<string, unknown> = {
      ...
      promptAssembly: {
        ...
        ...(traceRendered !== undefined && traceRendered.length > 0 ? { traceRendered } : {}),
        ...
        ...(timelineExported !== undefined && timelineExported.length > 0 ? { timelineExported } : {}),
