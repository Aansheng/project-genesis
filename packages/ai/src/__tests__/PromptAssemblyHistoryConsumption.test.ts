...
function getHistory(request: { metadata?: Record<string, unknown> }): Record<string, unknown> | undefined {
  const assembly = getAssembly(request)
  return assembly?.history as Record<string, unknown> | undefined
}
...
describe('BuilderOptions', () => {
  it('should accept promptAssemblyHistoryBuilder field', async () => {
    ...
    expect(getHistory(request)).toBeDefined()
  })

  it('should allow promptAssemblyHistoryBuilder to be omitted', async () => {
    const builder = new DefaultPromptBuilder([new UserInputModule()], {
      promptAssemblyTraceBuilder: new DefaultPromptAssemblyTraceBuilder(),
    })
    const request = await builder.build(createPipelineContext())
    expect(getHistory(request)).toBeUndefined()
  })

  it('should allow promptAssemblyHistoryBuilder to be undefined', async () => {
    const builder = new DefaultPromptBuilder([new UserInputModule()], {
      promptAssemblyTraceBuilder: new DefaultPromptAssemblyTraceBuilder(),
      promptAssemblyHistoryBuilder: undefined,
    })
    const request = await builder.build(createPipelineContext())
    expect(getHistory(request)).toBeUndefined()
  })
  ...
  it('should be backward compatible with existing fields', async () => {
    ...
    expect(assembly?.history).toBeUndefined()
  })

  it('should accept promptAssemblyHistoryBuilder in full setup', async () => {
    const builder = new DefaultPromptBuilder([new UserInputModule()], fullSetup)
    const request = await builder.build(createPipelineContext())
    expect(getHistory(request)).toBeDefined()
  })
...
describe('Builder invocation', () => {
  it('should invoke builder when both trace and builder are present', async () => {
    const builder = new DefaultPromptBuilder([new UserInputModule()], fullSetup)
    const request = await builder.build(createPipelineContext())
    expect(getHistory(request)).toBeDefined()
  })
  ...
  it('should not invoke builder when history builder is missing', async () => {
    const builder = new DefaultPromptBuilder([new UserInputModule()], {
      promptAssemblyTraceBuilder: new DefaultPromptAssemblyTraceBuilder(),
    })
    const request = await builder.build(createPipelineContext())
  ...
  it('should receive the current trace', async () => {
    let capturedTraces: unknown
    const trackingBuilder: PromptAssemblyHistoryBuilder = {
      build(traces) {
        capturedTraces = traces
        return { entries: [{ index: 0, trace: {} }] }
      },
    }
    const builder = new DefaultPromptBuilder([new UserInputModule()], {
      promptAssemblyTraceBuilder: new DefaultPromptAssemblyTraceBuilder(),
      promptAssemblyHistoryBuilder: trackingBuilder,
    })
    await builder.build(createPipelineContext())
    expect(capturedTraces).toBeDefined()
    expect(Array.isArray(capturedTraces)).toBe(true)
    expect((capturedTraces as Array<unknown>)).toHaveLength(1)
  })

  it('should receive trace array with one element', async () => {
    let capturedTraces: unknown
    const trackingBuilder: PromptAssemblyHistoryBuilder = {
      build(traces) {
        capturedTraces = traces
    ...
    await builder.build(createPipelineContext())
    const traces = capturedTraces as Array<{ strategy?: { name: string } }>
    expect(traces[0].strategy).toBeDefined()
  })

  it('should preserve custom builder output', async () => {
    const customBuilder: PromptAssemblyHistoryBuilder = {
      build() {
    ...
    const builder = new DefaultPromptBuilder([new UserInputModule()], {
      promptAssemblyTraceBuilder: new DefaultPromptAssemblyTraceBuilder(),
      promptAssemblyHistoryBuilder: customBuilder,
    })
    const request = await builder.build(createPipelineContext())
    const history = getHistory(request)
    expect(history?.entries).toHaveLength(1)
    expect((history?.entries as Array<{ index: number }>)[0].index).toBe(42)
  })
})
...
describe('Metadata creation', () => {
  it('should store history in metadata when builder is present', async () => {
    const builder = new DefaultPromptBuilder([new UserInputModule()], fullSetup)
    const request = await builder.build(createPipelineContext())
    expect(getHistory(request)).toBeDefined()
  })

  it('should not store history when builder is missing', async () => {
    const builder = new DefaultPromptBuilder([new UserInputModule()], {
      promptAssemblyTraceBuilder: new DefaultPromptAssemblyTraceBuilder(),
    })
    const request = await builder.build(createPipelineContext())
  ...
  it('should not store history when trace builder is missing', async () => {
    const builder = new DefaultPromptBuilder([new UserInputModule()], {
      promptAssemblyHistoryBuilder: new DefaultPromptAssemblyHistoryBuilder(),
    })
    const request = await builder.build(createPipelineContext())
  ...
  it('should store history with correct shape', async () => {
    const builder = new DefaultPromptBuilder([new UserInputModule()], fullSetup)
    const request = await builder.build(createPipelineContext())
    const history = getHistory(request)
  ...
  it('should store history with one entry', async () => {
    const builder = new DefaultPromptBuilder([new UserInputModule()], fullSetup)
    const request = await builder.build(createPipelineContext())
    const history = getHistory(request)
    expect((history?.entries as Array<unknown>)).toHaveLength(1)
  })

  it('should store history entry with trace', async () => {
    const builder = new DefaultPromptBuilder([new UserInputModule()], fullSetup)
    const request = await builder.build(createPipelineContext())
    const history = getHistory(request)
    const entry = (history?.entries as Array<{ trace: unknown }>)[0]
    expect(entry.trace).toBeDefined()
  })

  it('should store history entry with index 0', async () => {
    const builder = new DefaultPromptBuilder([new UserInputModule()], fullSetup)
    const request = await builder.build(createPipelineContext())
    const history = getHistory(request)
    const entry = (history?.entries as Array<{ index: number }>)[0]
...
describe('Deterministic', () => {
  it('should produce same history across multiple builds', async () => {
    const builder = new DefaultPromptBuilder([new UserInputModule()], fullSetup)
    const r1 = await builder.build(createPipelineContext())
    const r2 = await builder.build(createPipelineContext())
    expect(getHistory(r1)).toEqual(getHistory(r2))
  })

  it('should produce same history across different builder instances', async () => {
  ...
  it('should produce same history for same inputs', async () => {
    const builder = new DefaultPromptBuilder([new UserInputModule()], fullSetup)
...
describe('Stateless', () => {
  ...
  it('should produce independent history results', async () => {
    const builder = new DefaultPromptBuilder([new UserInputModule()], fullSetup)
    const r1 = await builder.build(createPipelineContext())
    const r2 = await builder.build(createPipelineContext())
    expect(getHistory(r1)).not.toBe(undefined)
    expect(getHistory(r2)).not.toBe(undefined)
  })

  it('should produce fresh history for each build', async () => {
    const builder = new DefaultPromptBuilder([new UserInputModule()], fullSetup)
    const r1 = await builder.build(createPipelineContext({ input: 'a' }))
    const r2 = await builder.build(createPipelineContext({ input: 'b' }))
    expect(getHistory(r1)).toBeDefined()
    expect(getHistory(r2)).toBeDefined()
  })
})
...
describe('Pure', () => {
  ...
  it('should produce same prompt regardless of history builder presence', async () => {
...
describe('Legacy constructor', () => {
  it('should not produce history with legacy constructor', async () => {
  ...
  it('should not produce history with BuilderOptions without history builder', async () => {
    const builder = new DefaultPromptBuilder([new UserInputModule()], {
      promptAssemblyTraceBuilder: new DefaultPromptAssemblyTraceBuilder(),
    })
    const request = await builder.build(createPipelineContext())
  ...
  it('should produce history with full BuilderOptions', async () => {
    const builder = new DefaultPromptBuilder([new UserInputModule()], fullSetup)
    const request = await builder.build(createPipelineContext())
    expect(getHistory(request)).toBeDefined()
  })
...
describe('No prompt changes', () => {
  it('should produce identical prompt with and without history builder', async () => {
    const builderWith = new DefaultPromptBuilder([new UserInputModule()], fullSetup)
    const builderWithout = new DefaultPromptBuilder([new UserInputModule()], {
      strategyEvaluator: new DefaultStrategyEvaluator(),
      strategies: [new DefaultPromptStrategy()],
  ...
  it('should not inject history into prompt output', async () => {
    const builder = new DefaultPromptBuilder([new UserInputModule()], fullSetup)
    const request = await builder.build(createPipelineContext())
    expect(request.prompt).not.toContain('history')
  })

  it('should have history only in metadata', async () => {
    const builder = new DefaultPromptBuilder([new UserInputModule()], fullSetup)
    const request = await builder.build(createPipelineContext())
    expect(getHistory(request)).toBeDefined()
    expect(request.prompt).not.toContain('history')
  ...
  it('should not have history in prompt text', async () => {
    const builder = new DefaultPromptBuilder([new UserInputModule()], fullSetup)
    const request = await builder.build(createPipelineContext())
    expect(request.metadata?.promptAssembly).toHaveProperty('history')
    expect(request.prompt).toBeDefined()
...
describe('History validation', () => {
  it('should have entries array', async () => {
    const builder = new DefaultPromptBuilder([new UserInputModule()], fullSetup)
    const request = await builder.build(createPipelineContext())
    const history = getHistory(request)
    expect(Array.isArray(history?.entries)).toBe(true)
  ...
  it('should have entry with index 0', async () => {
    const builder = new DefaultPromptBuilder([new UserInputModule()], fullSetup)
    const request = await builder.build(createPipelineContext())
    const history = getHistory(request)
    expect((history?.entries as Array<{ index: number }>)[0].index).toBe(0)
  })

  it('should have entry with trace containing strategy', async () => {
    const builder = new DefaultPromptBuilder([new UserInputModule()], fullSetup)
    const request = await builder.build(createPipelineContext())
    const history = getHistory(request)
    const entry = (history?.entries as Array<{ trace: { strategy?: { name: string } } }>)[0]
    expect(entry.trace.strategy).toBeDefined()
    expect(entry.trace.strategy?.name).toBe('default')
  })

  it('should have entry with strategy name matching selected strategy', async () => {
    const builder = new DefaultPromptBuilder([new UserInputModule()], fullSetup)
    const request = await builder.build(createPipelineContext())
    const history = getHistory(request)
    const entry = (history?.entries as Array<{ trace: { strategy?: { name: string } } }>)[0]
  ...
  it('should have entry with trace containing plan', async () => {
    const builder = new DefaultPromptBuilder([new UserInputModule()], fullSetup)
    const request = await builder.build(createPipelineContext())
    const history = getHistory(request)
    const entry = (history?.entries as Array<{ trace: { plan?: unknown } }>)[0]
    expect(entry.trace.plan).toBeDefined()
  })

  it('should have entry with trace containing snapshot', async () => {
    const builder = new DefaultPromptBuilder([new UserInputModule()], fullSetup)
    const request = await builder.build(createPipelineContext())
    const history = getHistory(request)
    const entry = (history?.entries as Array<{ trace: { snapshot?: unknown } }>)[0]
