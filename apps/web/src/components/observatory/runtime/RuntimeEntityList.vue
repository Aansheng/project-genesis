<script setup lang="ts">
import { ref } from 'vue'
import type { RuntimeEntityViewModel } from '../../../adapters/observatory'

type RuntimeEntity = RuntimeEntityViewModel

const props = defineProps<{
  entities: readonly RuntimeEntity[]
  selectedId: string
}>()

const emit = defineEmits<{
  select: [id: string]
}>()

const itemRefs = ref<HTMLButtonElement[]>([])

function setItemRef(el: unknown, index: number): void {
  if (el) itemRefs.value[index] = el as HTMLButtonElement
}

function onKeydown(event: KeyboardEvent): void {
  const current = props.entities.findIndex((e) => e.id === props.selectedId)
  let next = current

  if (event.key === 'ArrowDown') {
    next = Math.min(current + 1, props.entities.length - 1)
  } else if (event.key === 'ArrowUp') {
    next = Math.max(current - 1, 0)
  } else if (event.key === 'Home') {
    next = 0
  } else if (event.key === 'End') {
    next = props.entities.length - 1
  } else {
    return
  }

  event.preventDefault()
  const entity = props.entities[next]
  if (!entity) return
  emit('select', entity.id)
  itemRefs.value[next]?.focus()
}

function select(entity: RuntimeEntity): void {
  emit('select', entity.id)
}
</script>

<template>
  <nav
    class="runtime-entity-list"
    aria-label="Entity list"
    @keydown="onKeydown"
  >
    <h2 class="runtime-entity-list-title">
      Entity List
    </h2>
    <ul class="runtime-entity-list-items">
      <li
        v-for="(entity, index) in entities"
        :key="entity.id"
        class="runtime-entity-list-item"
      >
        <button
          :ref="(el) => setItemRef(el, index)"
          type="button"
          class="runtime-row"
          :class="{ 'runtime-row--active': entity.id === selectedId }"
          :aria-current="entity.id === selectedId ? 'true' : undefined"
          @click="select(entity)"
        >
          <span class="runtime-row-id">
            {{ entity.id }}
          </span>
          <span class="runtime-row-type">
            {{ entity.type }}
          </span>
        </button>
      </li>
    </ul>
  </nav>
</template>

<style scoped>
.runtime-entity-list {
  display: flex;
  flex-direction: column;
  min-width: 0;
  border-right: 1px solid var(--obs-border, #232327);
  background: var(--obs-surface, #111113);
  overflow-y: auto;
}

.runtime-entity-list-title {
  margin: 0;
  padding: var(--obs-space-3, 12px);
  border-bottom: 1px solid var(--obs-border, #232327);
  font-size: 11px;
  font-weight: 600;
  letter-spacing: 0.06em;
  text-transform: uppercase;
  color: var(--obs-text-dim, #63636d);
}

.runtime-entity-list-items {
  list-style: none;
  margin: 0;
  padding: var(--obs-space-1, 4px);
  display: flex;
  flex-direction: column;
  gap: var(--obs-space-1, 4px);
}

.runtime-entity-list-item {
  margin: 0;
  padding: 0;
}

.runtime-row {
  display: flex;
  flex-direction: column;
  align-items: flex-start;
  width: 100%;
  gap: var(--obs-space-1, 4px);
  padding: var(--obs-space-2, 8px) var(--obs-space-3, 12px);
  border: 1px solid transparent;
  border-radius: var(--obs-radius-s, 6px);
  background: transparent;
  color: var(--obs-text-muted, #a1a1aa);
  text-align: left;
  cursor: pointer;
  transition:
    background-color 0.12s ease,
    border-color 0.12s ease;
}

.runtime-row:hover {
  background: var(--obs-surface-2, #161618);
  color: var(--obs-text, #f5f5f4);
}

.runtime-row:focus-visible {
  outline: 2px solid var(--obs-accent, #6e7bff);
  outline-offset: -2px;
}

.runtime-row--active,
.runtime-row--active:hover {
  background: var(--obs-accent-soft, rgba(110, 123, 255, 0.14));
  border-color: rgba(110, 123, 255, 0.35);
  color: var(--obs-text, #f5f5f4);
}

.runtime-row-id {
  font-family: var(--obs-font-mono, 'SF Mono', 'Fira Code', Consolas, monospace);
  font-size: 13px;
  font-weight: 500;
}

.runtime-row-type {
  font-size: 11px;
  color: var(--obs-text-dim, #63636d);
}
</style>