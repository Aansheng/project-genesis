...
**Sprint 12** - AI World Generation (WO-S12-004 complete)
...
| Architecture Version | v1.114 (Sprint 12) |
| Last Completed WO | WO-S12-004 - Structured Model Client Integration Foundation |
...
| Current End-to-End Pipeline | Natural Language → Studio command lifecycle → IntentRouter → GameIntentExtractor → optional OpenAI StructuredGenerationClient → unknown candidate → GameWorldValidator → GameWorldModel, with deterministic provider fallback → SemanticGameDslBuilder → RuntimeWorldStore → Studio/Pixi/Observatory |
...
- `OpenAIStructuredGenerationClient` is implemented behind `StructuredGenerationClient` using the existing OpenAI SDK.
- `VITE_AI_ENABLED` defaults to false; the web composition root selects OpenAI only when enabled, selected, and keyed.
...
Known remaining gap: provider credentials are intentionally environment-only and manual browser verification with a real key is pending.
