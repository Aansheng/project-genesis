...
**Sprint 14** - World Evolution Foundation (WO-S14-003 complete; targeted Runtime synchronization active)
**Current WO** - WO-S14-003 Targeted Runtime Mutation & Semantic Synchronization (code complete; browser verification recorded below)
...
| Item | Status |
| ----------------------- | --- |
| Status | Sprint 14 **World Evolution Foundation** is active; semantic and targeted Runtime application are complete while visual regeneration remains intentionally deferred |
| Architecture Version | v1.144 |
| Last Completed WO | WO-S14-003 Targeted Runtime Mutation & Semantic Synchronization |
| Current User-Visible Behavior | After a world is created, supported imperative evolution instructions are planned against the current semantic snapshot, applied atomically to the semantic authority, then synchronized by targeted ID operations into the live Runtime world. Replacement preserves Runtime IDs, position, velocity, gameplay components, and controller state; additions receive deterministic safe positions and primitive fallback rendering; removal updates the exact Runtime target; world properties such as `timeOfDay` report no Runtime impact. History, Diff, Timeline, Trace, Event Stream, Runtime, and World Graph show the committed state and explicit `Runtime synchronized` / `Runtime no runtime impact` / failure status. New worlds clear prior evolution projections. |
| Current End-to-End Pipeline | Genesis Studio → StudioCommandBar → Pinia `gameStore` semantic authority (`GameWorldModel` + properties + semantic revision) → IntentRouter (create-world / world-evolution) → captured semantic context → existing BrowserStructuredGenerationClient → AI gateway → structured candidate → candidate parse → deterministic target resolution → `WorldSemanticDelta` validation → `DefaultSemanticWorldDeltaApplier` → immutable semantic commit → `DefaultRuntimeWorldEvolutionSynchronizer` against the live Runtime snapshot → one `RuntimeWorldStore.setWorld` commit → existing visualization loop / Renderer / Observatory Runtime view. AssetManifest, AssetStore, image-generation queues, camera reset, player reset, and Runtime system recreation remain outside evolution synchronization. |
| Current Blocking Issue | No S14-003 blocker. Visual asset regeneration/rebinding for semantic replacements and additions remains intentionally deferred; primitive fallback and existing manifest entries remain truthful. |
| Product Verification | YES — live browser matrix completed for Farm create/move/replace/remove, RPG replace/add/remove, world-property no-impact, player continuity, Runtime/World Graph refresh, v1.144 Observatory lifecycle truth, unchanged asset counts, and empty browser error/warning logs. |
| Next Recommended Verification | Define a separate visual-asset evolution work order if semantic replacements/additions should regenerate or rebind visual resources. |
...
- Runtime sync lifecycle facts are emitted as
  ...
  Runtime and World Graph rebind from the live Runtime world; visual status
  remains explicitly pending and AssetManifest/image generation are unchanged.
