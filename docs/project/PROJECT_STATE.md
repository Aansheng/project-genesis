...
**Sprint 13** - Visual & Asset Generation (WO-S13-001 complete)
...
| Architecture Version | v1.125 (Sprint 13) |
| Last Completed WO | WO-S13-001 - Visual Design Specification Foundation |
| Current User-Visible Behavior | Genesis Studio behavior is unchanged: generated worlds still render with deterministic primitive Pixi geometry. The semantic generation path now also has a vendor-independent, deterministic GameDesignSpecification → VisualDesignSpecification compiler, but it is not yet connected to assets or Renderer output. |
| Current End-to-End Pipeline | Genesis Studio → StudioCommandBar → Pinia gameStore → DefaultCommandExecutor → DefaultCreateWorldRuntimeExecutor → DefaultCreateWorldPipeline.executeAsync → GameIntent → GameDesignPromptBuilder → BrowserStructuredGenerationClient → POST /api/world-generation → AI Gateway → AIProviderConfigurationService client → OpenAI-compatible StructuredGenerationClient → candidate → strict parse → GameWorldValidator → GameDesignSpecification → GameDesignWorldBuilder → GameWorldModel → SemanticGameDslBuilder → RuntimeProjection → RuntimeWorldStore → gameplay systems → Pixi; a separate deterministic GameDesignSpecification → VisualDesignSpecification compiler is available as the next semantic branch but is not yet invoked by runtime generation. Generation diagnostics are copied to the session Observatory trace, and terminal AI failures use deterministic fallback with truthful source/reason/attempt diagnostics. |
...
| Next Recommended Verification | Begin S13-002 AssetManifest and deterministic fallback asset boundary; keep VisualDesignSpecification separate from Renderer and Runtime. |
