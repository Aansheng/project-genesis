...
**Current WO** - WO-POST-S13-001 Runtime/Visual Geometry Alignment & Visual Scale Correctness (complete)
...
| Architecture Version | v1.140 (post-Sprint-13 correctness pass) |
| Last Completed WO | WO-POST-S13-001 Runtime/Visual Geometry Alignment & Visual Scale Correctness |
| Current User-Visible Behavior | Studio can select API-compatible game design or local Codex CLI from Settings. Visual generation runs FIFO across background, terrain, characters, NPCs, animals, and meaningful props; repeated semantic archetypes share one job and Studio/Observatory expose semantic labels and binding metadata. Generated terrain now fits the existing world terrain/platform bounds, and generated characters use readable semantic world-space envelopes with feet anchors. |
...
| Product Verification | Automated geometry/scale and Runtime collision verification passed. Browser verification of primitive-vs-generated terrain contact, generated player scale, and viewport resize remains pending for this post-freeze pass. |
| Next Recommended Verification | Run the browser scenario `创建一个冰雪主题的平台游戏，有一个玩家、两个敌人和一个 Boss`; compare primitive fallback vs generated assets, then resize the viewport and verify world scale remains stable. Sprint 14 remains reserved for world evolution. |
