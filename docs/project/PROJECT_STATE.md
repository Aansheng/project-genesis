...
**Sprint 12** - AI World Generation (WO-S12-012 complete)
...
| Last Completed WO | WO-S12-013 - AI Provider Configuration Foundation |
...
| Current End-to-End Pipeline | Natural Language → Studio command lifecycle → IntentRouter → GameIntentExtractor → BrowserStructuredGenerationClient → HTTP AI Gateway → server StructuredGenerationClient/OpenAI (budget + timeout) → bounded retry → unknown candidate → strict parse → GameWorldValidator → GameDesignSpecification → GameDesignWorldBuilder → GameWorldModel → SemanticGameDslBuilder → RuntimeWorldStore → Studio/Pixi/Observatory; terminal AI failures use deterministic fallback with truthful source/reason/attempt diagnostics. |
| Current Blocking Issue | Real OpenAI/provider verification is pending a configured key and running local server; settings are intentionally session-only; theme, difficulty, patrol, checkpoint, boss, and unsupported multiplayer semantics are not gameplay systems. |
