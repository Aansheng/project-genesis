...
**Sprint 14** - World Evolution Foundation (WO-S14-002 complete; Runtime synchronization deferred)
**Current WO** - WO-S14-002 Semantic World Delta Application Foundation (code complete; browser verification recorded below)
...
| Item | Status |
| ----------------------- | --- |
| Status | Sprint 14 **World Evolution Foundation** is active; semantic application is complete and Runtime synchronization remains a separate follow-up |
| Architecture Version | v1.143 |
| Last Completed WO | WO-S14-002 Semantic World Delta Application Foundation |
| Current User-Visible Behavior | After a world is created, supported imperative evolution instructions are planned against the current semantic snapshot and automatically applied to that semantic authority. Replacement preserves IDs, add allocates deterministic IDs, remove updates only semantics, and world-property updates change the paired semantic properties. History, Diff, Timeline, Trace, and Event Stream report `semantic_applied`; Runtime and World Graph remain unchanged Runtime projections and visibly report synchronization pending. New worlds clear prior evolution projections. |
| Current End-to-End Pipeline | Genesis Studio → StudioCommandBar → Pinia `gameStore` semantic authority (`GameWorldModel` + properties + semantic revision) → IntentRouter (create-world / world-evolution) → for evolution: captured semantic context → existing BrowserStructuredGenerationClient → AI gateway → structured candidate → candidate parse → deterministic target resolution → `WorldSemanticDelta` validation → `DefaultSemanticWorldDeltaApplier` → immutable semantic state commit → operation history/domain events → Observatory. RuntimeWorldStore, AssetManifest, AssetStore, Pixi, camera, and image-generation queues are intentionally not mutated by semantic application. Creation continues through semantic world → Game DSL → RuntimeWorldStore → Pixi. |
| Current Blocking Issue | Semantic World is updated, but Runtime targeted mutation, AssetManifest/AssetStore rebinding, Renderer synchronization, and visual regeneration are intentionally deferred to WO-S14-003. The temporary semantic/runtime divergence is expected and must remain explicit. |
| Product Verification | YES — live browser verification passed for Farm replace + follow-up remove, RPG merchant replacement, Add, Remove, Theme, Observatory session continuity, semantic IDs/revisions, Runtime non-mutation, and truthful Observatory projections. |
| Next Recommended Verification | WO-S14-003: define and implement targeted Runtime application for the already-applied semantic delta, preserving IDs and keeping Runtime/Renderer/asset status truthful. |
...
- Replacement preserves stable IDs; additions use slugged `name-1`,
  ...
  unsupported in v1.
- History, Diff, Timeline, Trace, and Event Stream include real semantic
  application lifecycle facts. Diff says semantic applied with Runtime
  synchronization pending; no Runtime, AssetManifest, AssetStore, Renderer,
  camera, or image-generation mutation occurs.
- Runtime and World Graph intentionally remain Runtime projections until
  WO-S14-003.
