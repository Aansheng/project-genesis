...
| Architecture Version | v1.118 (Sprint 12) |
| Last Completed WO | WO-S12-008 - AI Game Design Specification Foundation |
...
| Current End-to-End Pipeline | Natural Language → Studio command lifecycle → IntentRouter → GameIntentExtractor → optional BrowserStructuredGenerationClient → HTTP AI Gateway → server StructuredGenerationClient/OpenAI → unknown candidate → GameWorldValidator → GameDesignSpecification → GameDesignWorldBuilder → GameWorldModel, with deterministic provider fallback → SemanticGameDslBuilder → RuntimeWorldStore → Studio/Pixi/Observatory |
