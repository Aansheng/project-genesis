...
**Sprint 15** - Capability-Specific Generation Context & Runtime Gameplay Rules (WO-S15-005 code/product verification complete; WO-META-004 control-plane hardening complete)
**Current WO** - WO-META-004 Subagent Delegation & Lifecycle Hardening (completed; WO-S15-006 prepared READY, not executed)
...
| Item | Status |
| ----------------------- | --- |
| Status | WO-S15-005 remains code complete and browser-verified. WO-META-004 is a docs/control-plane-only hardening work order; the approved generic ENEMY STOMP path and Sprint 14 semantic → Runtime → visual → asset → renderer behavior remain authoritative. |
| Architecture Version | v1.152 |
| Last Completed WO | WO-META-004 Subagent Delegation & Lifecycle Hardening; last completed product WO remains WO-S15-005 |
| Current User-Visible Behavior | Creating a normal platformer produces a playable Runtime world, immutable `GameplaySpecification`, and world-bound immutable `GameplayRuleSet`. When the player enters an enemy from above, Runtime emits `ENTITY_CONTACT_STARTED` with `direction=top`; the generic `enemy-stomp` rule passes player/enemy/top conditions, commits `REMOVE_ENTITY(eventTarget)` plus `APPLY_VELOCITY(eventActor, y=-12)`, removes the enemy from Runtime and Renderer, and keeps the player moving, bouncing, re-landing, and jump-capable. Non-top contact is observable but does not execute the stomp. Deferred/partial/unsupported rules remain visibly non-executed; no score, damage, health, goal, spawn, timer, or arbitrary provider code executes. |
| Current End-to-End Pipeline | Genesis Studio → StudioCommandBar → Pinia `gameStore` semantic authority → IntentRouter → semantic world → Game DSL → Runtime projection → RuntimeWorldStore → `GameplaySpecification` → provider candidate validation / deterministic `GameplayRuleBuilder` → world-bound `GameplayRuleSet` → ordered Runtime systems with Runtime-owned position/collision geometry → finalized `GameplayEvent` batch with typed contact direction → generic matcher → supported player/enemy/top condition evaluator → staged generic `REMOVE_ENTITY` + `APPLY_VELOCITY` action executor → immutable Runtime World commit → Renderer natural entity sync → separate Gameplay Rule / Gameplay Event Observatory projections. |
| Current Blocking Issue | No blocker for WO-S15-005. Score/numeric state, damage/health, enemy AI, goals/win/lose, timers, spawn execution, property actions, unrelated rich multi-action transactions, and gameplay-rule evolution remain deferred. |
| Product Verification | YES — MANUAL local Studio browser verification on 2026-08-21: generated a 6-entity platformer, observed a non-top `direction=bottom` contact with `conditions_failed`, then a real `direction=top` contact with `enemy-stomp · executed · committed · REMOVE_ENTITY:executed, APPLY_VELOCITY:executed`; Runtime/Renderer fell from 6 to 5 entities with no enemy; player later emitted `ENTITY_LANDED`, remained controllable, and generated another `ENTITY_JUMPED`/landing sequence. Browser logs contained no errors or warnings. AUTO tests separately prove rollback, exactly-once, and World A/World B stale isolation. |
| Next Recommended Verification | WO-S15-006 is prepared READY for the approved DAMAGE/HEALTH scenario; begin it only in a new Supervisor turn after shared-contract review. Do not execute it as a side effect of WO-META-004. |
