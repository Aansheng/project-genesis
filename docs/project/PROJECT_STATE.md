...
| Architecture Version | v1.17 (Sprint 5) |
| Architecture Status | **Evolving** — Prompt Assembly History Export Consumption (WO-S5-083) complete — DefaultPromptAssemblyHistoryExporter wired into DefaultPromptBuilder at Phase 0.9599769, exporting history as pretty-printed JSON and storing at `metadata.promptAssembly.historyExported`. historyExported is additive, coexists with all existing fields. Metadata only — no prompt changes, no breaking changes. Architecture v1.17. |
