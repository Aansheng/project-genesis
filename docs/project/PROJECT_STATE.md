...
| Current Blocking Issue | No code blocker for S13-008B. DashScope result URLs expire after 24 hours and verified output is RGB without claimed alpha; generated artifact ownership/publication, background removal, AssetManifest wiring, terrain, background, and animation remain deferred. |
...
| Next Recommended Verification | With DashScope credentials available, set `IMAGE_AI_PROVIDER=dashscope`, POST a character request to `/api/image-generation`, inspect the normalized PNG URL/dimensions and operation, then plan S13-009 for durable artifact publication and transparency handling. |
