...
**Current WO** - WO-POST-S13-002 Deterministic World Layout & Fallback Visual Binding Correctness (code complete; browser verification partial)
...
| Last Completed WO | WO-POST-S13-002 Deterministic World Layout & Fallback Visual Binding Correctness |
...
| Product Verification | Browser Scenario A now passes after the minimal gravity/input-focus fix: player falls to `y=400`, remains settled, accepts horizontal input, jumps, and re-lands. Farm coordinates, RPG entity readability, World A → World B reset, clean console, and a real background `codex-cli` publish/apply path passed. Terrain timeout fallback also kept the static visual. Generated player replacement remains pending because the player `codex-cli` operation stayed running beyond the observed timeout windows. |
| Next Recommended Verification | Re-run only the provider-available player replacement lifecycle until `entity-player-1-primary` reaches `ready`; compare Runtime ID/position/velocity before and after. Sprint 14 remains reserved for world evolution. |
