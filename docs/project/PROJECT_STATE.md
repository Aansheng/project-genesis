...
**Sprint 15** - Capability-Specific Generation Context (WO-S15-001 complete; product verified)
**Current WO** - WO-S15-001 GameplaySpecification & Game Loop Domain Foundation (code complete)
...
| Item | Status |
| ----------------------- | --- |
| Status | Sprint 15 gameplay foundation is code complete and product verified; Sprint 14 semantic → Runtime → visual → asset → renderer → Observatory behavior remains frozen and authoritative |
| Architecture Version | v1.148 |
| Last Completed WO | WO-S15-001 GameplaySpecification & Game Loop Domain Foundation |
| Current User-Visible Behavior | Creating a platformer, survival, farm, or other world produces the existing playable Runtime world plus a sibling immutable `GameplaySpecification`. The specification describes the loop, player mechanics, interactions, goals, failures, progression, and spawn intent with truthful `supported`/`deferred`/`unsupported` status. Gameplay generation failure falls back to deterministic intent and never blocks world creation. Observatory shows only a small real gameplay summary; it does not claim deferred mechanics execute. |
| Current End-to-End Pipeline | Genesis Studio → StudioCommandBar → Pinia `gameStore` semantic authority (`GameWorldModel` + properties + semantic revision) → IntentRouter → existing semantic world generation → Game DSL → existing Runtime projection → RuntimeWorldStore → gameplay context builder (current semantic entities + capability catalog) → existing structured gateway/provider → candidate validator → immutable `GameplaySpecification` → session-scoped store sibling → truthful Observatory summary. Gameplay data does not enter Runtime systems or Renderer. |
| Current Blocking Issue | No blocker for WO-S15-001. Runtime still executes only the existing movement/jump/gravity/vertical-motion/ground-collision slice and basic entity mutation; collect, damage, enemy AI, goals, failure, timers, spawns, XP/levels/upgrades, and win/lose execution remain deferred. |
| Product Verification | YES — local Studio browser verification covered platformer, survival, and farm inputs, same-schema genre separation, world A → world B gameplay replacement/isolation, real Observatory gameplay summary, and empty browser console warning/error logs. |
| Next Recommended Verification | Start S15-002 only after choosing the smallest measured Runtime gameplay bottleneck; keep gameplay execution, persistence, and generated-code evaluation out of this foundation. |
...
- `@genesis/shared` owns the immutable `GameplaySpecification`, compact
  ...
  gameplay generation context. The catalog mirrors only behavior already wired
  in production Runtime: movement, jump, gravity, basic ground collision, and
  add/remove mutation.
