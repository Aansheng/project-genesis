...
**Current WO** - WO-POST-S13-003 Codex CLI Image Generation Terminal Lifecycle & Player Replacement Verification (code complete; generated-player verification partial)
...
| Item | Status |
| ----------------------- | --- |
| Status | Sprint 13 **Visual & Asset Generation — FROZEN**; this post-freeze correctness pass is complete and does not reopen Sprint 13 feature scope |
| Architecture Version | v1.141 (post-Sprint-13 correctness pass) |
| Last Completed WO | WO-POST-S13-003 Codex CLI Image Generation Terminal Lifecycle & Player Replacement Verification |
| Current User-Visible Behavior | Studio can select API-compatible game design or local Codex CLI from Settings. Generated and fallback worlds now use deterministic world-space layout: exactly one validated player starts inside the initial camera framing above the authoritative `groundY=400` floor, enters the gravity pipeline with an initial velocity, and the focusable game viewport accepts Arrow/Space input after entity selection. Repeated semantic entities receive distinct stable positions, and a generated Sprite replaces its primitive/static fallback only after it is usable. |
| Current End-to-End Pipeline | Genesis Studio → StudioCommandBar → Pinia gameStore → BrowserStructuredGenerationClient → AI gateway → runtime-selected API or Codex CLI StructuredGenerationClient → prompt/candidate parser/validator → GameWorldGenerationProviderAdapter → RuntimeWorldStore → Pixi; failures → deterministic fallback → Observatory trace with safe provider metadata. Image generation remains a separate ImageGenerationRequest → image gateway/provider → GeneratedAssetPublisher path. |
| Current Blocking Issue | Generated artifacts are session-owned only and disappear on AI server restart; no persistence, alpha extraction, transparency guarantee, terrain tiling/autotiling, or animation exists. The synchronous gateway cannot expose an intermediate publishing event, so publishing is represented by completed artifact metadata rather than simulated progress. Codex CLI player generation now has a bounded terminal timeout, but the real player prompt timed out before producing an image; generated-player replacement evidence remains pending. |
| Product Verification | Browser fallback, gravity/collision, Farm, RPG, World A → World B, clean console, real Codex background/terrain success, and real Codex player timeout/fallback all passed. The player operation no longer remains permanently running and the FIFO scheduler advances. A real generated player Sprite replacement with unchanged Runtime identity/position/collision was not obtained. |
| Next Recommended Verification | Use an existing operational image provider or a provider-available Codex player success path to verify fallback → generated player replacement and compare Runtime ID/position/velocity around the replacement boundary. Sprint 14 remains reserved for world evolution. |
