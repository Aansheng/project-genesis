...
| Architecture Version | v1.21 (Sprint 5) |
| Architecture Status | **Evolving** — Prompt Assembly Observatory Consumption (WO-S5-087) complete — DefaultPromptBuilder consumes PromptAssemblyObservatoryBuilder at Phase 0.959977, building unified observatory from all six observability artifacts (trace, timeline, history, traceSnapshot, timelineSnapshot, historySnapshot) and storing at `metadata.promptAssembly.observatory`. observatory is additive, coexists with all existing fields. Metadata only — no prompt changes, no breaking changes. Architecture v1.21. |
