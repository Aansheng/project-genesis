...
**Sprint 14** - World Evolution Foundation (WO-S14-001 complete; Runtime application deferred)
**Current WO** - WO-S14-001 World Evolution Intent, Semantic Delta & Operation History Foundation (complete and Product Verified)
...
| Item | Status |
| ----------------------- | --- |
| Status | Sprint 14 **World Evolution Foundation** is complete; Runtime application remains a separate follow-up and Sprint 13 remains frozen |
| Architecture Version | v1.142 |
| Last Completed WO | WO-S14-001 World Evolution Intent, Semantic Delta & Operation History Foundation |
| Current User-Visible Behavior | After a world is created, supported evolution instructions produce a validated semantic plan against the current semantic snapshot. History, Diff, Timeline, Trace, and Event Stream show the real operation and correlation facts; Runtime and World Graph remain unchanged and authoritative. New worlds clear prior evolution projections. |
| Current End-to-End Pipeline | Genesis Studio → StudioCommandBar → Pinia gameStore → IntentRouter (create-world / world-evolution) → for evolution: explicit current `GameWorldModel` context → existing BrowserStructuredGenerationClient → AI gateway → API/Codex structured candidate → candidate parse → deterministic target resolution → `WorldSemanticDelta` validation → operation history/domain events → Observatory; RuntimeWorldStore/AssetManifest/Pixi are intentionally not mutated by evolution planning. Creation continues through semantic world → Game DSL → RuntimeWorldStore → Pixi. |
| Current Blocking Issue | World evolution execution is not yet enabled: validated deltas are planning artifacts only, so Runtime, AssetManifest, and Renderer do not apply sheep/add/remove/property changes. This is intentional for WO-S14-001 and is the next product boundary. |
| Product Verification | YES — real browser verification created Farm with `cow-1`/`cow-2`/`cow-3`, planned “把所有牛改成羊”, showed the validated operation in History/Diff/Timeline/Trace/Event Stream, and confirmed Runtime still contained the original cows. |
| Next Recommended Verification | Define the separate Runtime application contract for validated `WorldSemanticDelta` operations, with explicit identity preservation, mutation boundaries, and post-apply Observatory projections. |
...
- `DefaultIntentRouter` recognizes supported current-world evolution language
  ...
  unknown.
- `DefaultWorldEvolutionPlanner` assembles a semantic-only prompt when the
  configured provider is available, parses the candidate, resolves actual
  current semantic IDs, validates an immutable delta, and records an
  operation. It never falls back to Create World and never applies a delta.
- The Web store owns the current semantic snapshot and world/session ID. A
  validated evolution request records real History, Diff, Timeline, Trace, and
  Event Stream projections. Creating the next world clears those projections.
- Runtime, AssetManifest, and Pixi remain unchanged after planning; the
  Observatory Runtime and World Graph continue to show the current world.
