...
**Sprint 13** - Visual & Asset Generation (WO-S13-010 complete)
...
| Last Completed WO | WO-S13-010 - Visual Generation Activity & Execution Trace Experience |
...
| Current Blocking Issue | Generated artifacts are session-owned only and disappear on AI server restart; no persistence, alpha extraction, transparency guarantee, animation, or multi-asset generation exists. The synchronous gateway cannot expose an intermediate publishing event, so publishing is represented by completed artifact metadata rather than simulated progress. |
| Product Verification | Automated lifecycle and renderer callback coverage is complete. Real Codex CLI browser verification remains pending for this WO's stage-by-stage UX and failure fixtures. |
| Next Recommended Verification | Start `IMAGE_AI_PROVIDER=codex-cli pnpm --filter @genesis/ai-server dev`, create a world, observe World created + Visuals generating, navigate to Full Observatory during generation, then verify Visuals ready only after the player Sprite is visible. |
