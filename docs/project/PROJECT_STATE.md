...
**Current WO** - Game Design Codex CLI Provider & User-Selectable AI Mode
...
| Current User-Visible Behavior | Studio can select API-compatible game design or local Codex CLI from Settings. Codex execution remains server-side; generation failures preserve the existing deterministic playable-world fallback. Image generation remains independently controlled by IMAGE_AI_* settings. |
...
| Next Recommended Verification | Open Settings, switch Game Design AI to Local Codex CLI, save and test availability, then create a world and verify the trace reports codex-cli. Switch back to API and confirm image-generation behavior is unchanged. |
