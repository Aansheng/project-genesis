...
| Architecture Version | v1.27 (Sprint 5) |
| Architecture Status | **Evolving** — Prompt Assembly Observatory Export Consumption (WO-S5-093) complete — `DefaultPromptBuilder` now consumes `PromptAssemblyObservatoryExporter` via the new optional `promptAssemblyObservatoryExporter` BuilderOptions field. Phase 0.9599778 exports the built observatory as a serialized string, stored at `metadata.promptAssembly.observatoryExported`. Metadata only — no prompt changes, no API breaking changes, backward compatible. Architecture v1.27. |
