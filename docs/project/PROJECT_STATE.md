...
| Architecture Version | v1.23 (Sprint 5) |
| Architecture Status | **Evolving** — Prompt Assembly Observatory Diff Consumption (WO-S5-089) complete — DefaultPromptBuilder consumes PromptAssemblyObservatoryDiffer at Phase 0.9599775, diffing the unified observatory against an empty baseline and storing the result at `metadata.promptAssembly.observatoryDiff`. observatoryDiff is additive, coexists with all existing fields. Metadata only — no prompt changes, no breaking changes. Architecture v1.23. |
