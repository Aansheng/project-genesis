...
**Sprint 15** - Capability-Specific Generation Context (WO-S15-000 complete; product verified)
**Current WO** - WO-S15-000 Capability-Specific Generation Context Foundation (code complete)
...
| Item | Status |
| ----------------------- | --- |
| Status | Sprint 15 context foundation is code complete; Sprint 14 semantic → Runtime → visual → asset → renderer → Observatory behavior remains frozen and authoritative |
| Architecture Version | v1.147 |
| Last Completed WO | WO-S15-000 Capability-Specific Generation Context Foundation |
| Current User-Visible Behavior | After a world is created, supported evolution instructions still operate on the current semantic authority and preserve Sprint 14 targeted Runtime/visual behavior. Each generation capability now receives a frozen minimum context: world evolution gets current semantic entities and revisions; image generation gets current visual/asset facts, canonical bindings, and bounded metadata-only continuity hints; game design gets its current request and capability constraints. Image requests use deterministic provider-neutral sections and Observatory shows only safe context scope, world, revisions, target, binding, and reference counts. |
| Current End-to-End Pipeline | Genesis Studio → StudioCommandBar → Pinia `gameStore` semantic authority (`GameWorldModel` + properties + semantic revision) → IntentRouter → capability-specific context builder → immutable minimum context → existing BrowserStructuredGenerationClient / BrowserImageGenerationClient → AI gateway/provider → structured candidate or image artifact → existing semantic delta / Runtime synchronizer / visual planner / canonical asset executor → targeted AssetManifest and AssetStore updates → existing Pixi renderer → safe Observatory projections. Stable entity IDs remain bindings; current semantic names remain authoritative. |
| Current Blocking Issue | None for WO-S15-000. Conversation memory/history, RAG/vector retrieval, reference-image transport, context caching, durable generated-asset persistence, and gameplay mechanics remain explicitly deferred. |
| Product Verification | YES — local Studio browser session created `world-1` with `cow-1..3`, evolved to Sheep, showed image context with Sheep plus the stable cow bindings and semantic/Runtime/visual revisions `1/1/1`, followed up with Merchant at `2/2/2`, then created `world-2` and confirmed old evolution/context records were isolated. Browser console had no warnings/errors. |
| Next Recommended Verification | Begin S15-001 Gameplay Mechanics Foundation; keep durable generated-asset lifecycle and richer reference-guided generation separate. |
...
- `@genesis/shared` owns immutable `WorldEvolutionGenerationContext`,
  ...
  deterministic builders. `GameplayGenerationContext<T>` is only a future
  typed extension point; no gameplay specification exists yet.
