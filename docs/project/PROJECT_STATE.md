...
| Architecture Version | v0.87 (Sprint 5) |
| Architecture Status | **Evolving** — Prompt Inspector Foundation (WO-S5-052) complete — PromptInspector + PromptInspectorSection + PromptInspectorBuilder + DefaultPromptInspectorBuilder. Maps snapshot fields (strategyRendered, strategySelection, strategyModule, plan, optimizedPlan, planDiff, planRendered) to labeled sections with consistent ordering. Foundation only — not consumed by PromptBuilder. No behavioral changes. No breaking changes. |
