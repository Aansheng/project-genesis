...
**Sprint 13** - Visual & Asset Generation (WO-S13-008B complete)
...
| Architecture Version | v1.133 (Sprint 13) |
| Last Completed WO | WO-S13-008B - DashScope Native Image Provider Integration |
| Current User-Visible Behavior | Generated Studio worlds resolve selected repository-owned player/enemy/boss/checkpoint fixtures into Pixi sprites. The server image gateway can execute OpenAI-compatible or native DashScope text-to-image and returns normalized generated-image results; generated images are not yet published into Studio assets. Unmapped or failed assets remain primitive visuals; movement and jump use unchanged Runtime geometry. |
| Current End-to-End Pipeline | Genesis Studio → StudioCommandBar → Pinia gameStore → DefaultCommandExecutor → DefaultCreateWorldRuntimeExecutor → DefaultCreateWorldPipeline.executeAsync → GameIntent → GameDesignPromptBuilder → BrowserStructuredGenerationClient → AI Gateway → candidate → GameWorldValidator → GameDesignSpecification → GameWorldModel → RuntimeProjection → RuntimeWorldStore → gameplay systems → Pixi; generation diagnostics → VisualDesignSpecificationBuilder → AssetSpecificationBuilder → StaticAssetCatalog → immutable AssetManifest → session AssetStore → PixiAssetAdapter → Sprite with primitive fallback. Entity IDs remain explicit through AssetManifest.entityId → RenderEntity.id. |
| Current Blocking Issue | No code blocker for S13-008B. DashScope result URLs expire after 24 hours and verified output is RGB without claimed alpha; the experimental Codex CLI image path depends on local CLI authentication and ImageGen availability. Generated artifact ownership/publication, background removal, AssetManifest wiring, terrain, background, and animation remain deferred. |
| Product Verification | YES for deterministic/unavailable-provider Studio path: fallback world reaches Runtime, mapped static Sprite renders, primitive fallback remains available, and clean browser console verification passes. Real provider scenario is not claimed because the configured provider was unavailable during verification. |
| Next Recommended Verification | For the local prototype, set `IMAGE_AI_PROVIDER=codex-cli` and POST a character request to `/api/image-generation`; verify the local CLI writes a PNG and the gateway returns a normalized data URI. Then plan S13-009 for durable artifact publication and transparency handling. |
