...
**Sprint 15** - Capability-Specific Generation Context & Runtime Gameplay Events (WO-S15-002 complete; product verified)
**Current WO** - WO-S15-002 Gameplay Event Model & Runtime Event Observation Foundation (code complete)
...
| Item | Status |
| ----------------------- | --- |
| Status | Sprint 15 GameplaySpecification and Runtime Gameplay Event foundations are code complete and product verified; Sprint 14 semantic → Runtime → visual → asset → renderer behavior remains authoritative |
| Architecture Version | v1.149 |
| Last Completed WO | WO-S15-002 Gameplay Event Model & Runtime Event Observation Foundation |
| Current User-Visible Behavior | Creating a world still produces the playable Runtime world and immutable `GameplaySpecification`. The Runtime now emits bounded normalized `ENTITY_JUMPED`, `ENTITY_LANDED`, `ENTITY_CONTACT_STARTED`, `ENTITY_ADDED`, and `ENTITY_REMOVED` facts through an ephemeral per-tick collector. Observatory Event Stream shows only those real Gameplay facts and World Evolution domain events; it does not show movement ticks or claim gameplay results. |
| Current End-to-End Pipeline | Genesis Studio → StudioCommandBar → Pinia `gameStore` semantic authority → IntentRouter → semantic world → Game DSL → Runtime projection → RuntimeWorldStore → ordered Runtime systems → `GameplayEventCollector` → immutable `ExecutionTickResult.gameplayEvents` → renderer observer bridge → bounded Observatory Event Stream. `GameplaySpecification` remains a sibling intent authority; no Trigger/Condition/Action execution is present. |
| Current Blocking Issue | No blocker for WO-S15-002. Runtime still does not execute collection, damage, enemy AI, goals, failure, timers, spawns, XP/levels/upgrades, or win/lose rules; events are facts only and have no gameplay result semantics. |
| Product Verification | YES — local Studio browser verification covered real jump/landing events, bounded Event Stream behavior, Runtime mutation events, World Evolution coexistence, unchanged movement/camera/rendering, and empty browser warning/error logs. |
| Next Recommended Verification | S15-003: introduce the smallest measured Trigger/Condition/Action-compatible interpretation only after a concrete event-driven gameplay scenario is selected. |
