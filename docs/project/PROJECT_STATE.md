...
| Architecture Version | v1.100 (Sprint 10) |
...
The production path uses `ObservatoryRuntimeBinding` on Observatory mount. `loadMockObservatory()` remains explicit for tests/demo fixtures only. Manual browser verification is pending because the local dev server could not be reached from the browser automation environment after restart.
