...
**Sprint 13** - Visual & Asset Generation (WO-S13-012 complete)
**Current WO** - WO-S13-012B Environment Queue Progression & Metadata Correctness
...
| Architecture Version | v1.138 (Sprint 13) |
| Last Completed WO | Game Design Codex CLI Provider & User-Selectable AI Mode |
| Current User-Visible Behavior | Studio can select API-compatible game design or local Codex CLI from Settings. Visual generation runs FIFO across background, terrain, and character assets; terminal environment results release queue capacity, and Studio/Observatory labels derive from asset metadata without fake environment entities. |
...
| Current Blocking Issue | Generated artifacts are session-owned only and disappear on AI server restart; no persistence, alpha extraction, transparency guarantee, tiling, or animation exists. Background/terrain visuals are single bounded resources, not a tileset or parallax system. The synchronous gateway cannot expose an intermediate publishing event, so publishing is represented by completed artifact metadata rather than simulated progress. Real Codex CLI success remains environment-dependent; the controlled timeout/fallback path is verified. |
| Product Verification | Code-level queue and metadata regressions are covered; browser verification of terrain leaving queued and the full Studio/Observatory checklist remains pending. |
| Next Recommended Verification | Create an environment-capable world, verify background → terminal releases terrain to generating, inspect truthful Background/Terrain metadata in Studio and Full Observatory, then repeat with a forced timeout and a superseding world. |
