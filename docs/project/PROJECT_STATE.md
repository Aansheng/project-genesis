...
| Architecture Version | v1.11 (Sprint 5) |
| Architecture Status | **Evolving** — Prompt Assembly History Consumption (WO-S5-077) complete — PromptAssemblyHistoryBuilder wired into DefaultPromptBuilder at Phase 0.9599767, building history from the current trace and storing at `metadata.promptAssembly.history`. history is additive, coexists with all existing fields. Metadata only — no prompt changes, no breaking changes. Architecture v1.11. |
