...
**Sprint 19** - Animated Entity Presentation (**ACTIVE**)
**Current WO** - WO-S19-001 — Runtime-Derived Player Presentation State Assets (**IN_PROGRESS; reopened P0 reachability repair; Code Complete = YES; Product Verified = NO**)
...
| Item | Status |
| ----------------------- | --- |
| Status | Sprint 15 through Sprint 18 are Code Complete = YES, Product Verified = YES, and FROZEN = YES. Sprint 18 froze at v1.164. Sprint 19 is ACTIVE under SPRINT_CONTINUOUS with WO-S19-001 IN_PROGRESS after a Human/CTO-requested P0 reachability repair; Code Complete = YES and Product Verified = NO pending real Studio observation. |
| Architecture Version | v1.166; Player presentation state assets are distinct and selected from Runtime-derived state, and active horizontal input now truthfully produces `VelocityComponent.x` for the existing Runtime motion path. Ground continues to consume existing `ground-repeat-x` over the Runtime-authoritative continuous ground plane and Platform remains locally bounded |
| Last Completed WO | WO-S18-004 — Ground-Repeat Composition Across the Authoritative Ground Plane; Sprint 17 FROZEN at v1.160 |
| Current User-Visible Behavior | A natural-language platformer request passes structural validation and the bounded baseline gate. A complete provider candidate is accepted; an incomplete or unavailable provider selects the deterministic fallback with truthful selection diagnostics. The Runtime owns `active`/`failed`/`completed`; lethal player damage pauses gameplay rules, and the Studio Respawn control requests same-world Health/velocity recovery while preserving current entities and progression. Real Codex CLI image generation now emits separate Player idle/run/jump requests; idle has reached `succeeded → published → manifest updated → resolved → Renderer applied`, while Human/CTO directly observed jumping switch the visible Player to the generated jump-state asset. Remaining run/stop/facing/landing acceptance observations are pending. Ground remains horizontally tiled across the Runtime-authoritative plane; Platform uses its exact local visual and bounds. Runtime geometry remains authoritative. |
| Current End-to-End Pipeline | Genesis Studio → StudioCommandBar → Pinia `gameStore` semantic authority → IntentRouter → semantic world → Game DSL with generic Health/collision bounds → Runtime projection → provider candidate structural validation → bounded platformer baseline completeness gate → accepted provider or deterministic fallback with typed selection diagnostics → `GameplaySpecification` → deterministic `GameplayRuleBuilder` → world-bound `GameplayRuleSet` → ordered Runtime systems with Runtime-owned position/collision geometry → finalized `GameplayEvent` batch → generic matcher/condition evaluator → trusted gameplay actions and Runtime session/progression commit → Runtime-derived velocity/presentation state → Renderer adapter → visual design → state-aware AssetSpecification → independent role/state image request/context/prompt → state-preserving manifest → state-selected/mirrored Pixi sprite with primitive fallback; Runtime geometry remains authoritative. |
| Current Blocking Issue | WO-S19-001 was reopened because the active production controller made horizontal Position movement reachable but did not produce non-zero production `Velocity.x`, making Runtime-derived run/facing unreachable through real input. The repair and real-system reachability regression pass; direct real Studio Product Verification remains pending. |
| Product Verification | WO-S18-004 remains Product Verified. WO-S19-001 Code Complete = YES; Product Verified = NO until the repaired real Studio sequence passes. The browser keyboard bridge remains a tooling limitation, not a product defect, and does not authorize Runtime/Input changes. |
| Next Recommended Verification | In the real Studio Game page, record stationary idle, move-right run, stop-to-idle, move-left run with correct mirror, jump, land-while-moving run, stop-after-movement idle, gameplay continuity, and clean console. Then run a fresh Sprint 19 gap analysis, select exactly one smallest measured next WO, and do not enter Sprint 20 automatically. |
