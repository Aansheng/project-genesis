...
**Sprint 14** - World Evolution Foundation (WO-S14-005 complete; targeted visual execution active)
**Current WO** - WO-S14-005 Targeted Asset Execution, Manifest Rebinding & Incremental Visual Synchronization (code complete; final browser verification recorded below)
...
| Item | Status |
| ----------------------- | --- |
| Status | Sprint 14 **World Evolution Foundation** is active; semantic application, targeted Runtime synchronization, canonical visual execution, targeted manifest/store mutation, and incremental renderer synchronization are implemented |
| Architecture Version | v1.146 |
| Last Completed WO | WO-S14-005 Targeted Asset Execution, Manifest Rebinding & Incremental Visual Synchronization |
| Current User-Visible Behavior | After a world is created, supported imperative evolution instructions are planned against the current semantic snapshot, applied to the semantic authority, synchronized by targeted ID operations into the live Runtime world, analyzed by the deterministic visual planner, and executed only for its canonical generation-required set. Explicit IDs present in the current semantic snapshot can target one entity even when peer entities share an archetype; explicit group language still targets the full group. Existing visuals remain visible while generation runs; successful artifacts are validated, targeted bindings are rebound in one manifest commit, affected AssetStore entries are invalidated generation-safely, and the existing Pixi path upgrades only affected bindings. Provider, resolution, renderer, stale, and supersession failures retain the previous visual/fallback without rolling back semantic or Runtime state. Observatory History, Diff, Timeline, Trace, Event Stream, Runtime, Generation Trace, and World Graph expose the real execution facts. |
| Current End-to-End Pipeline | Genesis Studio → StudioCommandBar → Pinia `gameStore` semantic authority (`GameWorldModel` + properties + semantic revision) → IntentRouter (create-world / world-evolution) → captured semantic context → existing BrowserStructuredGenerationClient → AI gateway → structured candidate → candidate parse → deterministic target resolution → `WorldSemanticDelta` validation → `DefaultSemanticWorldDeltaApplier` → `DefaultRuntimeWorldEvolutionSynchronizer` → one `RuntimeWorldStore.setWorld` commit → `DefaultVisualEvolutionPlanner` with current VisualDesignSpecification/AssetSpecification → `VisualAssetEvolutionExecutor` consuming the planner-owned canonical set → existing `VisualAssetGenerationScheduler` + BrowserImageGenerationClient → targeted AssetManifest rebind → targeted AssetStore invalidation/resolution → existing Pixi entity/environment renderer application → Observatory projections. Runtime loop, player identity/state, camera, and unrelated asset bindings remain preserved. |
| Current Blocking Issue | No S14-005 code blocker. Durable generated-asset persistence, binary decode/optimization, animation, undo, and multi-operation persistence remain deferred; current generated URIs remain session-owned gateway artifacts. |
| Product Verification | YES — real browser verification passed for Cow ×3 → Sheep with movement and jump/re-land during queued/generating work, one canonical Sheep request, successful Observatory lifecycle, partial shared-binding removal without generation, Merchant → Robot with one canonical request and preserved identity/position, Boss removal without generation, and empty browser error/warning logs. |
| Next Recommended Verification | Scope WO-S14-006 to durable generated-asset lifecycle only if that remains the next product bottleneck; do not expand S14-005 into animation, undo, or scheduler redesign. |
