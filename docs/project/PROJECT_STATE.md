...
**Sprint 14** - World Evolution Foundation (WO-S14-004 complete; visual delta planning active)
**Current WO** - WO-S14-004 Visual Delta Planning & Targeted Asset Impact Analysis (code complete; browser verification recorded below)
...
| Item | Status |
| ----------------------- | --- |
| Status | Sprint 14 **World Evolution Foundation** is active; semantic application, targeted Runtime synchronization, and visual delta planning are complete while visual asset execution remains intentionally deferred |
| Architecture Version | v1.145 |
| Last Completed WO | WO-S14-004 Visual Delta Planning & Targeted Asset Impact Analysis |
| Current User-Visible Behavior | After a world is created, supported imperative evolution instructions are planned against the current semantic snapshot, applied to the semantic authority, synchronized by targeted ID operations into the live Runtime world, then analyzed by the deterministic visual planner. The planner updates only immutable VisualDesignSpecification/AssetSpecification state, reports canonical archetype impact, binding-only changes, orphaned assets, and the exact generation-required set; it does not execute image generation or mutate AssetManifest/AssetStore. History, Diff, Timeline, Trace, Event Stream, Runtime, and World Graph show semantic, Runtime, and visual-planning facts. New worlds clear prior evolution projections and visual revision markers. |
| Current End-to-End Pipeline | Genesis Studio → StudioCommandBar → Pinia `gameStore` semantic authority (`GameWorldModel` + properties + semantic revision) → IntentRouter (create-world / world-evolution) → captured semantic context → existing BrowserStructuredGenerationClient → AI gateway → structured candidate → candidate parse → deterministic target resolution → `WorldSemanticDelta` validation → `DefaultSemanticWorldDeltaApplier` → `DefaultRuntimeWorldEvolutionSynchronizer` → one `RuntimeWorldStore.setWorld` commit → `DefaultVisualEvolutionPlanner` with current VisualDesignSpecification/AssetSpecification → targeted immutable visual/asset delta plan → existing visualization loop / Renderer / Observatory projections. AssetManifest, AssetStore, image-generation queues, camera reset, player reset, and Runtime system recreation remain outside visual delta planning. |
| Current Blocking Issue | No S14-004 code blocker. Canonical visual generation execution, AssetManifest/AssetStore mutation, targeted texture replacement, and renderer rebinding remain intentionally deferred to WO-S14-005; existing primitive fallback and manifest entries remain truthful. |
| Product Verification | YES — live browser matrix verified visual planning lifecycle, targeted Farm/RPG changes, Runtime/entity continuity, zero console errors, and unchanged image/manifest/store state for planning-only evolution. |
| Next Recommended Verification | WO-S14-005 — execute only the canonical generation-required visual delta through the existing scheduler, then apply manifest/store/renderer changes with the same binding and orphan semantics. |
...
### WO-S14-004 Current Behavior

- `DefaultVisualEvolutionPlanner` consumes the before/after semantic snapshots,
  applied semantic mutation, synchronized Runtime result, and the current
  VisualDesignSpecification/AssetSpecification. It reuses the existing
  `AssetGenerationPolicy` and canonical grouping identity; it has no provider,
  scheduler, renderer, AssetManifest, or AssetStore dependency.
- The plan distinguishes unchanged canonical assets, targeted additions,
  archetype replacement/regeneration, binding-only changes, removals/orphaned
  assets, world-level visual dependencies, unaffected asset IDs/archetypes, and
  the exact canonical generation-required set. Cow ×3 → Sheep is one planned
  generation requirement with three entity bindings.
- Theme changes are broad eligible visual impact. `timeOfDay` is typed as a
  background-only dependency because the current visual model has no separate
  time-of-day field for terrain or characters. No image-generation operation is
  started during evolution; AssetManifest and AssetStore remain unchanged.
- Visual specs are stored in the web session beside semantic and Runtime
  revision markers. Visual revision commits are immutable and targeted; a
  failed or stale visual plan retains the previous visual/asset specifications.
- Observatory adds `VISUAL_IMPACT_STARTED` → `VISUAL_DELTA_PLANNED`/`FAILED`,
  visual revision/planning facts, layered Diff entries, and explicit
  `Asset execution pending` or `no asset generation required` language.
