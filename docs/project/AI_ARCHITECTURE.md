...
> Project Genesis — AI Architecture Reference (v1.57)
> Primary reference for all AI development.

> **Historical reference notice (Sprint 25, 2026-08-27):** This document
> describes the historical PromptBuilder/strategy architecture and is not the
> current production wiring authority. Current Web runtime wiring uses the
> provider-backed CreateWorld/evolution/gameplay paths documented in
> `docs/project/PROJECT_STATE.md` and audited in
...
apps/web/src/pages/observatory/ObservatoryPage.vue    — route entry (/observatory), body class management
  └── apps/web/src/components/observatory/ObservatoryShell.vue    — grid layout + design tokens
        ...
        └── ObservatoryContent.vue   — Overview → <ObservatoryOverview /> ; Trace → <ObservatoryTraceViewer /> ; Timeline → <ObservatoryTimelineViewer /> ; History → <ObservatoryHistoryViewer /> ; Diff → <ObservatoryDiffViewer /> ; Runtime → <ObservatoryRuntimeViewer /> ; Event Stream → <ObservatoryEventStream /> ; else placeholder grid (Settings)
              ...
              └── history/
                    ├── ObservatoryHistoryViewer.vue   — root viewer: mock history builds + selectedId state
              ...
              └── runtime/
                    ├── ObservatoryRuntimeViewer.vue   — root viewer: mock runtime state + selectedId state
